...
import { DEFAULT_CTX, DEFAULT_CTX_ID, GROUP_URL, PRIVATE_CTX } from '../../defaults'
import EventBus from '../../event-bus'
import { translate } from '../../locales/dict'

const URL_WITHOUT_PROTOCOL_RE = /^(.+\.)\/?(.+\/)?\w+/

let updateGroupTabTimeout: ReturnType<typeof setTimeout> | null = null
let urlRuleHistory = {}
...
async function loadTabsFromGlobalStorage() {
  ...
  if (dataTabIndex !== -1) {
    return await loadTabsFromInlineData(tabs, dataTabIndex)
  }
...
function updateGroupTab(groupTab) {
  if (updateGroupTabTimeout) clearTimeout(updateGroupTabTimeout)
  updateGroupTabTimeout = setTimeout(() => {
    ...
    updateGroupTabTimeout = null
  }, 256)
}

function resetUpdateGroupTabTimeout() {
  if (updateGroupTabTimeout != null) clearTimeout(updateGroupTabTimeout)
}
