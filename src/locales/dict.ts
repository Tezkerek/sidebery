...
export class PluralTranslation implements Translation {
  constructor(
    ...
    public plur: number[] | number[][] | RegExp[]
  ) {}
}
 ...
 *  Get dict value
 **/
export function translate(id: keyof Locale, plurNum?: number | string): string {
  ...
  if (translation instanceof PluralTranslation) {
    if (plurNum === undefined) return id

    let i = 0
    for (; i < translation.plur.length; i++) {
      let range = translation.plur[i]
      if (range === plurNum) return translation.messages[i]

      if (range instanceof Array && range[0] <= plurNum && range[1] >= plurNum) {
        return translation.messages[i]
      }
      if (range instanceof RegExp && range.test(plurNum.toString())) {
        return translation.messages[i]
      }
    }

    // Return the last variant by default
    return translation.messages[i]
  }
